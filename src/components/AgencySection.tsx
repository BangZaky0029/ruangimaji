...
import { ChevronLeft, ChevronRight, Sparkles, Globe, Award, Users, ArrowRight } from 'lucide-react';
...
const AgencySection: React.FC<AgencySectionProps> = ({ onMemberClick }) => {
                    ...
                    >
                      ...
                      <div className="absolute bottom-8 inset-x-0 flex justify-center px-4">
                        <motion.button
                          ...
                          whileHover={{ scale: 1.05, backgroundColor: 'rgba(197, 160, 89, 0.9)' }}
                          whileTap={{ scale: 0.95 }}
                          onClick={(e) => {
                            e.stopPropagation();
                            onMemberClick?.(member);
                          }}
                          className="px-6 py-2.5 rounded-full bg-white/10 backdrop-blur-xl border border-white/20 text-[8px] md:text-[9px] font-bold uppercase tracking-[0.2em] text-white flex items-center gap-2 shadow-lg transition-colors hover:text-white"
                        >
