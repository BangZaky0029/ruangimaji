            ...
            >
              ...
              <div className="space-y-12">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {categories.map(cat => (
                    <button key={cat.id} onClick={() => { setSelectedCategory(cat.slug); setIsFilterOpen(false); }} className={`text-left py-5 px-6 rounded-2xl flex justify-between items-center transition-all border ${selectedCategory === cat.slug ? 'bg-white/15 border-white/20 text-[#c5a059]' : 'bg-white/5 border-white/5 text-white/50'}`}><span className="text-xl font-serif font-bold italic">{cat.name}</span>{selectedCategory === cat.slug && <ChevronRight size={18} />}</button>
                  ))}
                </div>
              </div>
            </motion.div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-8">
          ...
          <div className="hidden md:flex flex-col items-end gap-6">
            ...
            <div className="flex flex-wrap justify-end gap-2">
              ...
              {categories.map(cat => (
                <button key={cat.id} onClick={() => setSelectedCategory(cat.slug)} className={`px-5 py-2.5 rounded-full text-[9px] font-bold uppercase tracking-widest border transition-all whitespace-nowrap ${selectedCategory === cat.slug ? 'bg-[#c5a059] text-white border-[#c5a059] shadow-lg shadow-[#c5a059]/20' : 'border-[#2d2a26]/10 text-[#2d2a26]/40 hover:border-[#c5a059]/40'}`}>{cat.name}</button>
              ))}
            </div>
          </div>
        </div>
